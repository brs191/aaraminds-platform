# Workflow Design — aara-psql-expert

## Trigger & Inputs

Trigger: user submits an engagement brief. Inputs: schema-definitions, postgresql-knowledge, sql-drafts. [TODO architect: confirm trigger and input list.]

## Step Graph

1. Receive and validate the brief (engagement-scoped).
2. Call get_schema_context (schema_context_read).
3. Call search_sql_knowledge (sql_knowledge_search).
4. Call create_sql_draft (sql_draft_create) — write action, approval boundary applies.
...
- create_sql_draft: requires approval before execution (soft or hard (write action)).
