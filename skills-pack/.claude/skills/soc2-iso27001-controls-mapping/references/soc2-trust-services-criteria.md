...
Where multiple implementations could satisfy a control, the pack uses the simpler one. Where the stack has no native answer, the gap is documented honestly (not fudged).
...
For a small / personal-pack scope, CC1 is a documentation exercise; the artifacts are written, not extracted.
...
| Sub-criterion | Implementation | Evidence | Artifact |
|---|---|---|---|
| CC2.1 — Information requirements | Service description in `README.md`; ADRs | Git history | Pack-of-docs export per service |
...
| CC3.2 — Risk identification | Threat model per system; risk register | Threat-model docs; risk-register spreadsheet | See `mcp-go-threat-modeling` skill |
...
For threat-model evidence, the artifact is the per-system threat model doc; see `mcp-go-threat-modeling` for the MCP-server flavor; equivalent for non-MCP services.
...
| Sub-criterion | Implementation | Evidence | Artifact |
|---|---|---|---|
| CC4.1 — Ongoing monitoring | Quarterly freshness reviews per `../../../../validation/governance/freshness-cadence.md` | Refresh logs (or commits) | Freshness cadence completion record |
...
| Sub-criterion | Implementation | Evidence | Artifact |
|---|---|---|---|
| CC7.1 — Capacity / availability | KEDA autoscaling rules; resource sizing per `azure-microservices-cost-review` | Container Apps scaling history; Azure Monitor metrics | Scaling history export; baseline-load graphs |
| **CC7.2 — System monitoring & alerting** | Application Insights / Log Analytics; alerts in `infra/observability/`; SLO tracking per `azure-microservices-observability` | Alert configuration in Terraform; alert history in Azure Monitor | Active-alert list; alert-fire history with response timelines |
...
| Sub-criterion | Implementation | Evidence | Artifact |
|---|---|---|---|
| **CC8.1 — Change management process** | Every change via PR; required review per `pr-review-azure-microservices`; CI gates; environment-promoted via OIDC pipelines | GitHub PR data; CI run logs; deployment records | Quarterly export: PRs merged, approvers, CI status, deploy times |
...
|---|---|---|---|
| A1.1 — Availability requirements | SLO docs per `docs/slo.md` per service | SLO definitions | SLO inventory |
...
| Control | Implementation | Evidence | Artifact |
|---|---|---|---|
| PI1.1 — Processing accuracy | Input validation; idempotency keys per `microservices-api-design`; testing per `pr-review-azure-microservices` | Test coverage; idempotency-key usage in API code | Code-review evidence; coverage reports |
...
| P2 — Choice / consent | Consent management (typically frontend; this pack doesn't ship a CMP) | Consent records | Sample consent-event logs |
