...
**Note on versions:** ISO/IEC 27001:2022 is the current standard. 27001:2013 is being retired (transition deadline October 2025; deadline already passed at the time of writing — see `../../mcp-go-server-building/references/ecosystem-facts.md` for freshness re-verification). The mapping below uses 2022 terminology with 2013 control numbers cross-referenced where relevant.
...
| A.5.8 — Information security in project management | Threat-model-per-project requirement | Threat-model docs (see `mcp-go-threat-modeling`) | Threat models inventory |
...
| A.8.25–8.27 — Secure development lifecycle, secure coding | Code review per `pr-review-azure-microservices`; CI gates per `mcp-go-production-review` for MCP, similar for others | Code-review records; CI logs | Quarterly PR merge + review export |
| A.8.28 — Secure coding | Same as above + language-specific anti-patterns | Same | Same |
| A.8.29 — Security testing in development and acceptance | Security tests in CI; per `mcp-go-threat-modeling` for MCP servers | Test suites in CI | Security test inventory |
