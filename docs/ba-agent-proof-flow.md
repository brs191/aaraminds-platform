...
5. Runtime opens a trace record, optionally starts the run-scoped OTel `invoke_agent` root span, and writes `agent_started` and `manifest_validated` audit events.
...
10. Runtime records `ToolInvocation`, `AuditEvent`, and optional child OTel tool spans linked back through `aap.audit_event_id`.
