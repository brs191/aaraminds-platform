...
| Memory | Read-side contracts closed 2026-07-19; extraction spike still required | Consolidation (claim-key conflict gate + audited supersession) and retrieval provenance (`memory_retrieved` / `memory_query_denied` in the tamper-evident chain, fail-closed reads) are now implemented and proven in the harness — see Local Harness Coverage. Remaining: extraction quality. Mem0 OSS is actively maintained (v2.4.6, 2026-04-04, incl. Azure AI Search fixes; JSON-extraction fixes in v2.4.5) and shipped a new memory algorithm in April 2026 with vendor-reported gains on temporal (+29.6) and multi-hop (+23.1) queries `[VERIFY — vendor-reported]`. The Phase 2 extraction-quality spike now has concrete contracts to measure against: extracted records must carry valid citations and claim keys that survive the consolidation gate. |
...
- Memory consolidation gate (added 2026-07-19): at most one active record per `(engagement_id, claim_key)`. A write colliding with an active claim is denied unless it names that record in `supersedes_memory_id`; a valid supersede retires the old record and is audited as `memory_superseded` (proof fields `ConflictingClaimWriteDenied`, `SupersededRecordExcluded`, `SupersessionAudited`). Supersede targets must exist, be active, and share the record's engagement/agent scope.
- Memory retrieval provenance (added 2026-07-19): every in-scope memory read emits a `memory_retrieved` audit event listing the returned `memory_ids` in the tamper-evident chain; cross-engagement query attempts are audited as `memory_query_denied`; a read whose provenance cannot be audited fails closed and returns nothing (proof fields `MemoryRetrievalAudited`, `CrossEngagementQueryAudited`). A run can now show which memories steered it.
